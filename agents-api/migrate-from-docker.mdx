...
| `-p 3000:3000` (or a random host port) | The URL replaces the host port: `https://{instanceId}.agent37.app` for the template's `default_port`, `https://{instanceId}-{port}.agent37.app` for any other |
...
```text
Read https://www.agent37.com/docs/llms-full.txt. My app runs as Docker containers on my own servers.
Move it to Agent37. Each container I run becomes one instance: build its Dockerfile into a template
with `npx agent37 templates build --default-port <the port it serves>`, create instances from it with
POST /v1/instances, pass the container's environment as `env` (fetch a credential that expires from
my server at boot instead), and reach its default port at https://{instanceId}.agent37.app and any
other port at https://{instanceId}-{port}.agent37.app, both with the X-Agent37-Key header (a public
port for anything that must work without a key). Do not run Docker inside an instance: wherever my
code calls `docker run`, call POST /v1/instances instead and store the instance id where the
container id was. Keep data under /home/node, make the entrypoint safe to run again on that disk,
and turn on auto_sleep for anything idle between uses. Start with one container on the smallest
shape. My key is in AGENT37_API_KEY.
```
