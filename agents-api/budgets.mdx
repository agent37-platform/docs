...
<Note>
  An instance top-up raises a ceiling; it does not move money. Funding the wallet itself happens in the dashboard at [agent37.com/dashboard/cloud/billing](https://www.agent37.com/dashboard/cloud/billing). See [Billing](/agents-api/billing).
</Note>
...
Composio calls meter at \$0.000114 each, counted against the instance's budget like any other managed call.

## Bring your own keys

Managed credentials are a convenience, not a requirement. To run a service on your own account, put your own provider key inside the instance with [exec](/agents-api/exec) or the agent's in-instance config. Calls made with your own keys go straight to the provider and never touch the managed meter or the budget.
