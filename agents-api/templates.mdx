...
System templates track the newest platform image: their `image_ref` moves forward as new versions ship, and each create uses whatever the template points at in that moment. Existing instances are unaffected — an instance keeps the image it was created from until you [update it](/agents-api/instances#update). If you need every instance you create to be identical — say a setup script runs after each create and depends on exact versions — pin the image instead: read the current tag with `GET /v1/templates/agent37-hermes`, [register its `image_ref` as a workspace template](#register-a-workspace-template), and create instances from that name.
