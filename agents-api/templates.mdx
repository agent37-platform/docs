...
Templates come in two scopes. `system` templates are the built-in catalog, the same for every workspace and read-only. `workspace` templates are ones you publish from your own image — a public registry reference or a [cloud build](#build-a-template-in-the-cloud) — visible only to your workspace.
...
| `POST` | `/v1/template-builds` | `201` with the new build and a presigned context `upload_url` |
| `POST` | `/v1/template-builds/{id}/start` | `200` `{ "id": "...", "status": "building" }` |
| `GET` | `/v1/template-builds/{id}` | `200` with the build object |
| `GET` | `/v1/template-builds/{id}/logs?offset=N` | `200` with the build object plus a log chunk |
...
## Build a template in the cloud

The path for a local or private image is a cloud build: send a Dockerfile and its build
context, and Agent37 builds the image on its own infrastructure and publishes it as a
workspace template. No local Docker, and nothing has to touch a public registry. The
supported client is the Agent37 CLI, run in the folder with your Dockerfile:

```bash
export AGENT37_API_KEY=sk_live_...
npx agent37 templates build ./my-agent --name my-agent --default-port 8000
```

The directory defaults to the current one and `--name` to the directory's name;
`--default-port` means [the same as on register](#register-a-workspace-template). The build
log streams live, and the command exits once the template is published, printing the applied
`revision` and `image_digest`. Ctrl-C detaches the log stream but never cancels the build —
it finishes server-side; check on it with `GET /v1/template-builds/{id}`.

<Warning>
  The CLI packs the **entire directory** as the build context — every file a `COPY` could
  need, not just the Dockerfile. `.git` never ships, and plain patterns from a `.dockerignore`
  at the context root are excluded (negations like `!keep-me` are skipped). Everything else
  goes: a stray `.env` in the folder ships with the context, and a `COPY . .` bakes it into
  the image. Check the folder, or add a `.dockerignore`, before you build.
</Warning>

The build contract:

- The `Dockerfile` sits at the context root, and the packed, gzipped context stays under
  100 MB (`400 invalid_request`) — the Dockerfile plus the files it COPYs, not your data.
- The build runs on `linux/amd64` with network access for package installs. `FROM` images
  must be **publicly pullable**: the build environment holds no registry credentials, so it
  cannot pull private base images.
- One build runs per workspace at a time (`409 build_conflict`), and a build has 45 minutes
  to finish (`build_timeout`). Builds are free.
- Built images up to about 4 GB are supported today; if you need more,
  [talk to the team](https://cal.com/vishnukool/30min).
- A finished build creates the template at `revision: 1` or bumps the existing one — every
  finished build increments `revision`, even when the image digest is unchanged. A build-born
  template has an `image_digest` and no `image_ref`. Existing instances are never updated.

### The build API

The build API is four endpoints; use them directly to implement the same flow in another
client. `POST /v1/template-builds` registers the build and returns where to send the context:

<ParamField body="name" type="string" required>
  The template to publish, following the same rules as [register](#register-a-workspace-template).
  A name that does not exist yet is created at `revision: 1` when the build succeeds; an
  existing workspace template gets a new revision.
</ParamField>

<ParamField body="size_bytes" type="integer" required>
  Byte count of the gzipped context you will upload, `1` through `100,000,000`. The size cap
  is re-checked against the actual uploaded bytes at `start`.
</ParamField>

<ParamField body="default_port" type="integer">
  Same meaning as on [register](#register-a-workspace-template); applied when the build
  succeeds.
</ParamField>

<ParamField body="description" type="string">
  Optional free-text description; applied when the build succeeds.
</ParamField>

```bash curl
tar --exclude .git -czf context.tgz -C ./my-agent .
BYTES=$(wc -c < context.tgz | tr -d ' ')
...
Upload the gzipped context to `upload_url` — one presigned `PUT`, valid for an hour, that
takes no Agent37 key. Clear curl's default `Expect: 100-continue` header; it is incompatible
with these presigned uploads:

```bash curl
curl --fail-with-body -X PUT -H 'Expect:' \
  --data-binary @context.tgz "$(jq -r .upload_url build.json)"
```

Then start the build. Starting before the context is uploaded returns `400 invalid_request`;
starting the same build twice, or while another build is running in the workspace, returns
`409 build_conflict`:
...
Progress is poll-driven: poll every few seconds until `status` is `succeeded` or `failed`.
`GET /v1/template-builds/{id}` returns the build object; `GET
/v1/template-builds/{id}/logs?offset=N` returns the same object plus a log chunk — `data` is
up to 256 KB of log starting at byte `offset`, and the returned `offset` is where to resume:
...
```json response
{
  ...
  "name": "my-agent",
  "status": "succeeded",
  "template": { "name": "my-agent", "revision": 1, "image_digest": "sha256:9b2e...c41f" },
  "created": 1784192400,
  "started": 1784192410,
  "finished": 1784192530,
  "offset": 4096,
  "data": "== Building image (linux/amd64)\n..."
}
```
...
<ResponseField name="status" type="string">
  `created` (waiting for the context upload and start) → `building` (your Dockerfile is
  building) → `ingesting` (the built image is being validated and copied into private
  storage) → `succeeded` or `failed`.
</ResponseField>
<ResponseField name="template" type="object">
  On `succeeded`: the published result — the template `name`, its new `revision`, and the
  image's `image_digest`.
</ResponseField>
<ResponseField name="error" type="object">
  On `failed`: `code` and `message`. `build_failed` (a Dockerfile step failed — the log shows
  which), `build_timeout` (over the 45-minute limit), `invalid_request` (a broken context: not
  a readable `.tar.gz`, or no Dockerfile at its root), `invalid_image_archive` or
  `image_too_large` (the built image failed validation), or `image_ingest_failed` and
  `internal_error` (our side; retry, or contact support).
</ResponseField>
<ResponseField name="created" type="integer">
  Creation time in epoch seconds. `started` and `finished` mark the same for their
  transitions, `null` until they happen.
</ResponseField>

A build failure is not an HTTP error: the poll returns `200` with `status: "failed"` and the
error object above. Branch on `status`.

## Register a workspace template

`POST /v1/templates` registers an amd64 image under a name you choose. The platform runs it as a managed instance with the same lifecycle, billing, [exec](/agents-api/exec), and routed port URLs as the catalog; what the image serves on its ports is up to it. The image can be anything, built from scratch or on [the Hermes base image](#build-on-the-hermes-base-image). Give the image as an `image_ref` — a public registry reference; omitting it returns `400 invalid_request`. For a local or private image, use a [cloud build](#build-a-template-in-the-cloud) instead: it creates or updates the template itself, so there is nothing to register afterwards. (`upload_id`, the retired direct-upload field, is rejected with `400 invalid_request`.) If you need an image larger than the 8 GB cap allows, [talk to the team](https://cal.com/vishnukool/30min).
...
Registration copies the image into private storage before it returns, so `POST` and `PATCH` are synchronous and can take up to a few minutes for a large image. A reference that isn't publicly pullable or has no `linux/amd64` image is rejected with `400 invalid_request`, and an image over the cap with `400 image_too_large` — so a broken image never reaches instance create.
...
Run `npx agent37 templates build` in the Dockerfile's folder to [build and publish it in the cloud](#build-a-template-in-the-cloud), or build and push it to a public registry yourself and register the `image_ref`. OpenClaw builds have the same shape: the FROM target is [`ghcr.io/agent37-platform/openclaw-base`](https://github.com/orgs/agent37-platform/packages/container/package/openclaw-base), published with the same dated tags as `agent37-openclaw`.

See [Build a custom image](/agents-api/custom-image) for the step-by-step walkthrough, with a copy-able example — a `Dockerfile`, an example skill, and a tiny bring-your-own-model proxy.
...
`PATCH /v1/templates/{name}` changes any of `name`, `image_ref`, `description`, or `default_port` (send `null` to clear it back to the `3737` fallback). Send at least one field. A changed `image_ref` is copied into private storage, so an image update is synchronous like create. To publish a new local build instead, run `npx agent37 templates build` again — a [cloud build](#build-a-template-in-the-cloud) onto the same name is the update. Each successful image update increments the platform-controlled `revision`; a finished build increments it even when the new image has the same `image_digest`. Sending the exact same `image_ref` string reuses the existing snapshot and does not increment it. Renaming or changing only `description` or `default_port` also leaves it unchanged. Renaming onto an existing name returns `409 template_conflict`; system templates return `403`.
...
Point `image_ref` at an older tag (or re-run the build from an older context) to roll back. Existing instances keep running on the image they already pulled; [update each instance](/agents-api/instances) to recreate it from the template's current image.
...
A workspace template registered from a public reference echoes that reference as `image_ref` and adds the pinned `image_digest`; one published by a [cloud build](#build-a-template-in-the-cloud) has an `image_digest` and no `image_ref`:
...
<ResponseField name="revision" type="integer | null">
  The platform-controlled workspace image revision. It starts at `1` and increments for a changed `image_ref` or every finished [cloud build](#build-a-template-in-the-cloud), even when the digest is unchanged. Metadata-only updates and the same exact `image_ref` leave it unchanged. `null` for system templates.
</ResponseField>
<ResponseField name="image_ref" type="string">
  For system templates and workspace templates registered from a registry, the image reference. **Absent** on a workspace template published by a cloud build — it lives only in Agent37's private storage, identified by `image_digest`.
</ResponseField>
...
| Status | Code | When |
| --- | --- | --- |
| `400` | `invalid_request` | Bad `name`, `image_ref`, or `default_port`; a missing `image_ref` (or the retired `upload_id`/`ports` fields); a registry reference that isn't publicly pullable or has no `linux/amd64` image; a direct image reference passed as `template` on instance create; a build context over 100 MB, or a build started before its context upload |
| `400` | `invalid_image_archive` | A built image failed validation: not a readable single `linux/amd64` image |
...
| `404` | `not_found` | Unknown template name or build id; repeat `DELETE` |
| `409` | `template_conflict` | Create or rename onto a name that already exists |
| `409` | `build_conflict` | The build was already started, or another build is running in the workspace — one at a time |
...
A failed build is reported on the build object, not as an HTTP error: the poll returns `200` with `status: "failed"` and `error.code` set to `build_failed`, `build_timeout`, one of the validation codes above, or `internal_error` for a failure on Agent37's side. See [Errors](/agents-api/errors) for the envelope and the full catalog.
