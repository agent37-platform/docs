...
The `agent37-` prefix is reserved for system templates. The catalog has one entry today, the default:

| Template | What it runs | Ports |
| --- | --- | --- |
| `agent37-hermes` | Hermes, the general agent: chat, browsing, code, files. | `3737` (default), `9119`, `7681`, `8080` |

<Note>
  More system templates (OpenClaw, Claude Code, Codex) are coming; they will appear in `GET /v1/templates` when they are available. Passing any other `agent37-` name on instance create returns `400 invalid_request`, because the prefix is reserved.
</Note>
...
```dockerfile
FROM ghcr.io/agent37-platform/hermes-base:2026.06.11c
...
<ResponseField name="status" type="string">
  `active` (resolvable on instance create) or `inactive` (a deactivated workspace template).
</ResponseField>
...
| `409` | `template_unavailable` | Instance create or update against an inactive workspace template |
