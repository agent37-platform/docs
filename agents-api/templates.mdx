...
```json response
{
  ...
  "mode": "multipart",
  "multipart_id": "opaque-r2-upload-id",
...
<ResponseField name="abort_url" type="string">
  A presigned `DELETE` the client can call after a failed upload. Hard-abandoned multipart
  uploads are also removed by bucket lifecycle.
</ResponseField>
...
header; it is incompatible with these R2 presigned uploads:

```bash curl
PART_URL=$(jq -r '.parts[0].upload_url' upload.json)
PART_BYTES=$(jq -r '.parts[0].size_bytes' upload.json)
curl --fail-with-body -H 'Expect:' -H "Content-Length: $PART_BYTES" \
