...
Registration validates the reference's shape and stores the template. The image is pulled lazily, the first time you create or update an instance from the template. A bad reference surfaces there as a failed create: the create returns `502 provisioning_failed` and costs nothing, since the meter only starts once an instance runs.
