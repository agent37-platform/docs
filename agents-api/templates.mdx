...
Templates come in two scopes. `system` templates are the built-in catalog, the same for every workspace and read-only. `workspace` templates are ones you register from your own image — a public registry reference or a [local upload](/agents-api/custom-image#from-a-local-image-no-registry) — visible only to your workspace.
...
| `POST` | `/v1/template-uploads` | `201` `{ "id": "up_...", "upload_url": "...", "expires_at": ... }`, a slot to upload a local image (see [From a local image](/agents-api/custom-image#from-a-local-image-no-registry)) |
...
`POST /v1/templates` registers an amd64 image under a name you choose. The platform runs it as a managed instance with the same lifecycle, billing, [exec](/agents-api/exec), and routed port URLs as the catalog; what the image serves on its ports is up to it. The image can be anything, built from scratch or on [the Hermes base image](#build-on-the-hermes-base-image). Give the image as **either** an `image_ref` (a public registry reference) **or** an `upload_id` (a [local upload](/agents-api/custom-image#from-a-local-image-no-registry)) — exactly one; sending both or neither returns `400 invalid_request`. If you need an image larger than the 5 GB cap allows, [talk to the team](https://cal.com/vishnukool/30min).
...
<ParamField body="upload_id" type="string">
  The `id` from a `POST /v1/template-uploads` slot whose archive you have uploaded (see [From a local image](/agents-api/custom-image#from-a-local-image-no-registry)). Use this for a private or unpublished image. The archive must be a `docker save` tarball (gzip optional), `linux/amd64`, and at most 5 GB. Mutually exclusive with `image_ref`; a template registered this way has no `image_ref`, only an `image_digest`.
</ParamField>
...
<ParamField body="default_port" type="integer">
  The port the bare instance URL (`https://{instanceId}.agent37.app`) routes to, and the port the platform probes at boot to decide the create succeeded. An integer 1 to 65535; the reserved ports `7681`, `8080`, `6080`, `7890`, `9119` are rejected (they belong to the managed runtime). Omitted, the bare URL falls back to `3737` — right for images built on [the Hermes base image](#build-on-the-hermes-base-image), whose gateway serves it — and boot is not probed, so an image that listens on nothing (a private sandbox driven by [exec](/agents-api/exec)) still creates fine. Every other port needs no declaration at all: it is already reachable at `https://{instanceId}-{port}.agent37.app`. With [auto-sleep](/agents-api/instances#auto-sleep) on, a sandbox nothing ever requests only wakes by an explicit [`start`](/agents-api/instances#start).
</ParamField>
...
Build it for `linux/amd64`, push it to a public registry, register it as a workspace template, and create instances from it. OpenClaw builds have the same shape: the FROM target is [`ghcr.io/agent37-platform/openclaw-base`](https://github.com/orgs/agent37-platform/packages/container/package/openclaw-base), published with the same dated tags as `agent37-openclaw`.
...
`PATCH /v1/templates/{name}` changes any of `name`, `image_ref` (or `upload_id`, for a new local image), `description`, or `default_port` (send `null` to clear it back to the `3737` fallback). Send at least one field; `image_ref` and `upload_id` remain mutually exclusive. Changing the image re-copies it into private storage, so a `PATCH` that touches it is synchronous like create. Renaming onto an existing name returns `409 template_conflict`; system templates return `403`.
