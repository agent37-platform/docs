...
`PATCH /v1/templates/{name}` changes any of `name`, `image_ref` (or `upload_id`, for a new local image), `description`, or `default_port` (send `null` to clear it back to the `3737` fallback). Send at least one field; `image_ref` and `upload_id` remain mutually exclusive. A new `image_ref` or any fresh `upload_id` is copied into private storage, so a `PATCH` that changes the image is synchronous like create. Sending the exact same `image_ref` string reuses the existing snapshot; publish a new tag when the bytes change. Renaming onto an existing name returns `409 template_conflict`; system templates return `403`.
