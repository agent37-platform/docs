...
The `agent37-` prefix is reserved for system templates. The catalog has two entries today:

| Template | What it runs | Ports |
| --- | --- | --- |
| `agent37-hermes` | Hermes, the general agent: chat, browsing, code, files. | `3737` (default), `9119`, `7681`, `8080` |
| `agent37-hermes-small` | Hermes, lean: chat, code, files, shell. No browser or desktop. | `3737` (default), `7681`, `8080` |

`agent37-hermes-small` is the only template that can run on the sub-floor 1 vCPU / 2 GB and 1 vCPU / 3 GB shapes, from \$2.74 per month; see [sizing](/agents-api/instances). Standard and custom templates start at 2 vCPU / 4 GB.
