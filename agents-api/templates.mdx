...
Point `image_ref` at an older tag — or, for a build-born template, re-run the [build](#build-an-image-in-the-cloud) from the older source — to roll back. Existing instances keep running on the image they already pulled; [update each instance](/agents-api/instances) to recreate it from the template's current image.
