...
A template is a prebuilt image that decides which agents an instance installs and which agents the gateway can route sessions to. Pass `template` when you create an [instance](/agents-api/instances), or omit it for the default.
...
<Note>
  **Multi-agent templates.** When a template installs more than one agent (like `all`), the gateway routes a new session to whichever agent you choose with the `agent` field on its [first message](/agents-api/messages). The choice is fixed for the session's life. Single-agent templates ignore it.
</Note>
