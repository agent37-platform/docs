...
openapi: "/openapi/hosting-api.json POST /v1/instances/{id}/signed-url"
---
...
The instance must be running, and `port` must be one it exposes (any port from the `ports` array, named or not). An optional `ttl_seconds` sets how long the URL lives, default `3600` (one hour), clamped to `[60, 86400]` (one minute to one day); pass a short value for a quick preview link or the max for a day-long share link. An unexposed or missing port, or a `ttl_seconds` that is not a positive integer, returns `400`; an unknown or cross-workspace instance returns `404`.
