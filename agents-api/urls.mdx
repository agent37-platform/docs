...
You can also serve these on your own domain: register a [custom domain](/agents-api/domains) and the instance URL (plus every public-port hostname) is mirrored at `{label}.yourdomain.com` with the same authentication. Preview URLs stay on `agent37.app`.
