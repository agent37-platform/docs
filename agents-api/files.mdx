...
`DELETE /v1/files?path=…` removes the path recursively and by force, like `rm -rf`: a directory and everything under it goes in one call. There is no confirmation and no guard, so a wrong `path` is gone from the disk; the way back is [restoring a backup](/agents-api/instances#restore), which rolls the whole instance back to that point and drops everything written since. A symlink is removed itself, not followed. The response is `{ "ok": true }`.
...
<Warning>
  Delete is recursive and unguarded. It removes whatever the key can reach, including directories full of files, and the only undo is [restoring a backup](/agents-api/instances#restore). Double-check `path` before you send it.
</Warning>
