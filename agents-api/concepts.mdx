...
<Note>
  Instance URLs require auth on every request: the `Authorization` Bearer for server-side and programmatic use, or a time-boxed [signed URL](/agents-api/urls) to open one in a browser tab. An unauthenticated request gets a 401.
</Note>
