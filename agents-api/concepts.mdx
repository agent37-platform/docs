...
| Concept | What it is |
| --- | --- |
| **Instance** | An isolated, persistent computer that runs your agent. Built from a [template](/agents-api/templates). Lives until you delete it, metered per minute for what it holds: the full rate while running, disk alone while stopped or sleeping. One instance runs one container: if you run several today, each becomes its own instance. Create one per end user. |
