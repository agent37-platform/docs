...
<Steps>
  <Step title="Get a key and fund your wallet">
    ...
    Then top up your workspace wallet at [www.agent37.com/dashboard/cloud/billing](https://www.agent37.com/dashboard/cloud/billing). A one-time $1 credit covers your first instance (the smallest costs $4.94 per month, billed a day at a time), but the workspace is capped at one instance until your first top-up, and from the second instance on your balance must hold 30 days of compute for the whole fleet, so add at least $10 to leave room for a second instance and managed LLM calls. Top-ups are $5 to $1000.
...
<Note>
  Done experimenting? `DELETE /v1/instances/{id}` refunds the unused remainder of the instance's prepaid day to your wallet, prorated (a minimum of one hour is billed). See [Billing](/agents-api/billing).
</Note>
