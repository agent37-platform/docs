...
| `invalid_request` | 400 | A request field is invalid: bad JSON, an unsupported resource shape, a direct image ref where a template name belongs, both or neither of a [template](/agents-api/templates)'s `image_ref`/`upload_id`, a not-publicly-pullable registry reference, a lifecycle action in the wrong state, or an invalid [`update`](/agents-api/instances#update) body (any field other than `template`, or a `template` naming a different base template). |
| `invalid_image_archive` | 400 | A [template](/agents-api/templates) upload is not a single `linux/amd64` image: wrong architecture, not a `docker save` tarball, or more than one image in the archive. |
| `image_too_large` | 400 | A [template](/agents-api/templates) archive or staged image is over the 8 GB decimal cap (`8,000,000,000` bytes). |
| `upload_not_found` | 404 | A multipart upload cannot be found, or a template create names an `upload_id` with no completed upload in your workspace — never uploaded, expired, or another workspace's. |
