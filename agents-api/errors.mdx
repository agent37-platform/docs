...
<Info>
  Both planes take the same `sk_live_` Bearer key, but their error envelopes differ: the Agent API adds optional `param` and `hint` fields. See [Core concepts](/agents-api/concepts) for the two planes.
</Info>
