...
| `insufficient_balance` | 402 | The wallet cannot cover a month of compute: at create (billed in advance) or at `start` of a `past_due` instance. |
| `instance_limit_reached` | 409 | The workspace is at its instance limit (default 25). |
| `capacity_unavailable` | 409 | `start` only: the pinned host cannot re-reserve this instance's CPU and RAM right now. |
...
<AccordionGroup>
  <Accordion title="insufficient_balance (402): the workspace wallet is empty" icon="wallet">
    The workspace wallet cannot cover a charge. You see it in three places: at create, when the first month (billed in advance) cannot be debited; at `start`, when a `past_due` instance's lapsed month cannot be re-debited; and inside agent behavior, when a managed call (LLM, Brave search, Composio) finds the wallet empty. The fix is the same everywhere: top up the wallet at `https://www.agent37.com/dashboard/cloud/billing` ($5 minimum, $1000 max per top-up). See [Billing](/agents-api/billing).
  </Accordion>
  ...
  <Accordion title="The capacity trio: instance_limit_reached, capacity_unavailable, no_capacity" icon="server">
    Three different walls, three different fixes. `instance_limit_reached` (409, create): the workspace is at its instance limit (default 25); delete instances you no longer need. `no_capacity` (503, create): no host can fit the requested shape right now; retry with backoff or pick a smaller shape, and a create that fails this way does not keep your money. `capacity_unavailable` (409, start): a stopped instance stays pinned to the host that holds its disk, and that host cannot re-reserve the CPU and RAM right now; retry later. See [Instances](/agents-api/instances).
  </Accordion>
</AccordionGroup>
