...
- **Automatic top-up is on by default and follows your top-up.** The Add funds dialog sets the refill rule next to the amount you are adding: buy the same amount again once the balance drops below half of it (a \$25 top-up refills \$25 below \$12.50; a \$5 top-up refills \$5 below \$3). Change the two numbers or turn it off before you pay, or any time after. See [Automatic top-up](#automatic-top-up).
...
Automatic top-up keeps instances from being suspended when the balance runs low: when it falls below your threshold, your card is charged for a fixed amount. The rule is checked the moment a charge takes the balance below the threshold, every hour, and once more right before any instance would be suspended, so instances stay up as long as the card charges.

- It is on by default and follows your top-up. Each time you add funds, the rule becomes "buy the same amount again once the balance drops below half of it": \$5 refills \$5 below \$3, \$10 refills \$10 below \$5, \$100 refills \$100 below \$50. The two numbers and the switch sit next to the Add button, and the Stripe payment page restates them, so you see what will be charged before you pay. Edit them or turn it off any time at [dashboard/cloud/billing](https://www.agent37.com/dashboard/cloud/billing).
- The threshold can be \$3 to \$1000. The purchase amount follows the usual \$5 to \$1000 top-up range.
