...
Start with the default. Performance is enabled per workspace: pass `"type": "performance"` on [create](/agents-api/instances#create-an-instance), and until your workspace has it, create returns `403 performance_unavailable` with a link to a short call where we turn it on.
