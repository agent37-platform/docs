...
<ParamField body="resources" type="object">
  The instance shape: `{ "cpu": 2, "memory": 4, "disk": 6 }`. Standard templates use one of the three shapes below; `agent37-hermes-small` additionally unlocks the sub-floor 1 vCPU / 2 GB and 1 vCPU / 3 GB shapes. Disk is any whole number of GB within the shape's range, and defaults to the range minimum when omitted. Any other combination returns `400 invalid_request` listing the valid shapes.
</ParamField>
