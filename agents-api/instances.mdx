...
`POST /v1/instances` returns `201` with the full instance object once `status` is `running`. Every field is optional, so a `POST` with no body works: you get the default template (`agent37-hermes-small`) on its smallest shape, 1 vCPU / 2 GB.

<ParamField body="template" type="string" default="agent37-hermes-small">
  A template name. `agent37-hermes-small` (the lean image) is the default; `agent37-hermes` (full, with browser and desktop) is the other system template. You can also pass one of your own [workspace templates](/agents-api/templates) by name. Free workspaces (before your first top-up) can create only `agent37-hermes-small`; any other template returns `403 tier_limit` until you top up. Unknown names return `400 invalid_request`. Direct image references are rejected with `400`: register a template first, then pass its name.
</ParamField>

<ParamField body="resources" type="object">
  The instance shape, for example `{ "cpu": 2, "memory": 4, "disk": 6 }`. Omitted, it uses the template's smallest shape: 1 vCPU / 2 GB on `agent37-hermes-small`, 2 vCPU / 4 GB on standard templates. `agent37-hermes-small` unlocks the sub-floor 1 vCPU / 2 GB and 1 vCPU / 3 GB shapes; standard templates use one of the three shapes below. Free workspaces are limited to the 1 vCPU shapes; a larger shape returns `403 tier_limit` until you top up. Disk is any whole number of GB within the shape's range, and defaults to the range minimum when omitted. Any other combination returns `400 invalid_request` listing the valid shapes.
</ParamField>
...
| --- | --- | --- | --- |
| 1 | 2 GB | 6-20 GB | \$2.74 per month (`agent37-hermes-small` only) |
...
```json response
{
  ...
  "status": "running",
  "template": "agent37-hermes-small",
  "image_ref": "ghcr.io/agent37-platform/hermes-small:2026.06.14",
  "resources": { "cpu": 1, "memory": 2, "disk": 6 },
  "ports": [
...
| `403 tier_limit` | Create or resize asks for a template or shape your plan does not include. Free workspaces run only `agent37-hermes-small` at the 1 vCPU shapes; a top-up unlocks the rest. |
