...
<ParamField body="template" type="string">
  A template name. `agent37-hermes` (the default) and `agent37-openclaw` are available today; `agent37-claude-code`, `agent37-codex`, and `agent37-all` are coming soon. Register your own under [custom templates](/agents-api/templates#custom-templates). See [Templates](/agents-api/templates).
</ParamField>
...
<Info>
  `agent37-hermes` and `agent37-openclaw` are available today, plus workspace custom templates in preview. The `agent37-claude-code`, `agent37-codex`, and `agent37-all` templates are coming soon. See [Templates](/agents-api/templates).
</Info>
