...
<ParamField body="template" type="string" default="agent37-hermes">
  A template name. `agent37-hermes` is the default and currently the only system template. You can also pass one of your own [workspace templates](/agents-api/templates) by name; an unknown name returns `400 invalid_request`, and an inactive workspace template returns `409 template_unavailable`. Direct image references are rejected with `400`: register a template first, then pass its name.
</ParamField>
...
<CardGroup cols={2}>
  ...
  <Card title="Billing" icon="credit-card" href="/agents-api/billing">
    The wallet, daily compute prepay, and refunds.
  </Card>
</CardGroup>
