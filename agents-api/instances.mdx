...
<ParamField body="type" type="string" default="default">
  `default` is what every create uses, and for most harnesses it is all you need. `performance` runs the instance on dedicated cores, for heavy work such as computer use, at 4x the default compute rate. Start with the default. See [Default and Performance instances](/agents-api/billing#default-and-performance-instances).
</ParamField>
