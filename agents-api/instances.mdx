...
<ParamField body="template" type="string" default="agent37-hermes-small">
  A template name. `agent37-hermes-small` (the lean image) is the default; `agent37-hermes` (full Hermes, with browser and desktop) and `agent37-openclaw` (OpenClaw, with a headless browser) are the other system templates. You can also pass one of your own [workspace templates](/agents-api/templates) by name. Unknown names return `400 invalid_request`. Direct image references are rejected with `400`: register a template first, then pass its name.
</ParamField>
