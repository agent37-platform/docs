...
<Steps>
  <Step title="One instance per user, on signup">
    ...
    Omitting `template` gives you `agent37-hermes`, the default, on the default 2 vCPU / 4 GB RAM / 8 GB disk shape, billed at $4.99 per month from your workspace wallet (see [Billing](/agents-api/billing)). The $1 `budget.topup_micros` grants one-time managed-spend headroom so the agent's LLM calls work from the first message; without it the per-instance [budget](/agents-api/budgets) defaults to $0.

    The call is synchronous and returns `201` with `status: "running"`, so the agent is ready the moment the call comes back. Store `inst.id` on the user row.
  </Step>
