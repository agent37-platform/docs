...
<Info>
  You talk to two base URLs. `https://api.agent37.com` creates and manages instances. Each instance then serves its own chat API at `https://{instanceId}.agent37.app`: the instance id is the hostname. The same `sk_live_` key authenticates both. See [Instance URLs](/agents-api/urls).
</Info>
...
<Warning>
  Route every call through your server. The `sk_live_` key is workspace-scoped: it can read and manage every instance and session in your workspace, not just one user's. It must never reach the browser. Your frontend talks to your backend; your backend holds the key and calls both base URLs.
</Warning>
...
<CardGroup cols={2}>
  <Card title="Send a message" icon="send" href="/agents-api/chat">
    The full `POST /v1/responses` contract: fields, the response object, and models.
  </Card>
  <Card title="Streaming" icon="activity" href="/agents-api/streaming">
    Render text, reasoning, and tool activity live, and reconnect dropped streams.
  </Card>
  <Card title="Sessions" icon="history" href="/agents-api/sessions">
    List, retrieve with history, and delete the threads on an instance.
  </Card>
  <Card title="Budgets" icon="wallet" href="/agents-api/budgets">
    Cap each user's managed spend with a monthly cap and one-time top-ups.
  </Card>
</CardGroup>
