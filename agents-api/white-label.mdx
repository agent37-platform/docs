...
I want to ship a branded product: a multi-tenant dashboard with chat, files, app integrations, and one-click access to each agent's terminal.
Clone https://github.com/agent37-platform/starter-kit and follow the guide's Get started steps (npm run setup asks for my sk_live_ key and a Supabase access token; ask me for the token), then set my product name and logo (ask me for them).
Done when npm run dev serves the dashboard and I can sign up, create an agent, and watch a reply stream in its Chat tab.
