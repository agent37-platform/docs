...
I have an agent37-hermes instance with a Hermes webhook subscription and want it to receive deliveries on a public URL without exposing its Agent API.
Hermes listens on port 8644: create a public port on it with the prefix webhooks (POST /v1/instances/{id}/public-ports) and give me the public origin that replaces http://localhost:8644 in the URL Hermes shows.
Done when a POST to https://webhooks-{instanceId}.agent37.app/webhooks/<subscription-name> reaches Hermes without an Agent37 key.
