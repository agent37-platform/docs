...
<Note>
  The browser's built-in `EventSource` cannot send a POST body or an `Authorization` header, so it cannot start a stream here. Use `fetch` as above, in the browser and in Node.
</Note>
...
<CardGroup cols={2}>
  <Card title="Send a message" icon="send" href="/agents-api/chat">
    Every field on `POST /v1/responses`, plus the full response object.
  </Card>
  <Card title="Sessions" icon="history" href="/agents-api/sessions">
    List, read, and delete the conversations your streams run in.
  </Card>
  ...
  <Card title="Errors" icon="alert-triangle" href="/agents-api/errors">
    Every error code on both planes, and how to branch on them.
  </Card>
</CardGroup>
