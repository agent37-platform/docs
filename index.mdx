---
title: "Agents API"
description: "Launch a real AI agent with one API call: its own computer, tools, memory, and connected accounts."
---
...
| Resource | What it is |
| --- | --- |
| **Instance** `inst_` | The agent's own always-on computer. Built from a [template](/agents-api/templates), persistent until you delete it. Create one per end user. |
| **Session** `sess_` | One conversation on an instance. A [message](/agents-api/messages) starts one; reuse its `session_id` to continue. An instance can hold many. |
| **Response** `resp_` | One agentic turn: your input, the agent's work, its reply. [Stream](/agents-api/streaming) it live or fetch it by id. |
...
<Note>
  Looking for **OpenClaw**: channels, model providers, and self-hosting? Switch products in the selector at the top of the sidebar, or start at the [OpenClaw overview](/openclaw/overview).
</Note>
